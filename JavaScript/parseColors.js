...
function parseHTMLColor(color) {
    const extra = (c,n) => {
        return n===1 ? /^#?([a-f\d]{1})([a-f\d]{1})([a-f\d]{1})$/i.exec(c).slice(1,4).map(i=>parseInt(i+i,16))
            : /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(c).slice(1,4).map(i=>parseInt(i,16))
    }
    const [r,g,b] = color.includes("#") ?
        color.length >= 5 ? 
            extra(color,2)
        :
            extra(color,1)
    :
        extra( PRESET_COLORS[color.toLowerCase()],2);
    return {r,g,b};
}
