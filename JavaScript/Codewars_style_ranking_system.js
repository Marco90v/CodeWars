...
class User{
    constructor(){
      this._rank = -8;
      this._progress = 0;
      this._ranks = [-8,-7,-6,-5,-4,-3,-2,-1,1,2,3,4,5,6,7,8];
    }
    get progress(){
    ...
    incProgress(n){
      if (n>8 || n<-8 || n===0) throw new error("Rank input out of range");
      const d = this._ranks.indexOf(n) - this._ranks.indexOf(this._rank);
      if(n===this._rank){
          this._progress+=3;
      }else if(n<this._rank){
        if(d){
          this._progress+=1;
        }
      }else if(n>this._rank){
          const addProgress = 10 * d * d;
          this._progress+=addProgress;
      }
      while(this._progress>=100 && this._rank<8){
          this._progress-=100;
          this._rank++;
          this._rank===0 && this._rank++;
      }
      if(this._rank===8){
          this._progress=0;
      } 
